  ...
  function checkStatuses() {
    ...
    cy.get("table tbody tr").each(($row) => {
      cy.wrap($row).find("td").eq(5).invoke("text").then((statusText) => {
        ...
        if (status.includes("Unparse") || status.includes("Cancel")) {
          cy.wrap($row).find("td").eq(5).find("svg").click({ force: true });
          allSuccess = false;
    ...
    cy.then(() => {
      if (hasFailed) {
        cy.log("❌ A file failed. Script stopped.");
        return; // Exit gracefully
      }

      if (hasParsingFile) {
        cy.log("⏳ Some files are still parsing → waiting 10 minutes before re-check...");
        cy.wait(600).then(() => {
          cy.reload();
          cy.wait(5000);
          checkStatuses();
        ...
        cy.get("body").then(($body) => {
          if ($body.find(".ant-pagination-next:not(.ant-pagination-disabled) > .ant-pagination-item-link").length > 0) {
            cy.log("➡️ Moving to next page...");
            cy.get(".ant-pagination-next > .ant-pagination-item-link").click({ force: true });
            cy.wait(5000);
            checkStatuses();
          } else {
            cy.log("🏁 Monitoring complete. All files Success!");
          }
        });
      } else {
        cy.log("⚠️ Re-checking in 10 minutes...");
        cy.wait(600).then(() => {
          cy.reload();
          cy.wait(5000);
          checkStatuses();
